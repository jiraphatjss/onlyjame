from Setup_Metamask import *

def before_all(context):
    open_chrome_with_metamask(context)
    switch_window_handle(context)
    import_wallet_first_time(context)
    import_and_switch_to_test_wallet(context)
    switch_window_handle(context)
    # wait.until(EC.number_of_windows_to_be(2))
    # asd(context)
    # set_up_metamask(context)
    ...
    # wait.until(EC.element_to_be_clickable(
    #     (By.CSS_SELECTOR, ("[data-testid=popover-close]")))).click()
    # wait.until(EC.element_to_be_clickable(
